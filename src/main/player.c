...
static audio_element_handle_t _fatfs_stream_reader, _audio_decoder, _i2s_stream_writer;
...
static audio_element_handle_t create_i2s_writer() {
    ...
    i2s_writer_cfg.i2s_config.channel_format = I2S_CHANNEL_STEREO;
...
void tx_audioWorker(void *args) {
    ...
    while (true) {
        ...
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT
            ...
            if(msg.source == (void *)_i2s_stream_writer) {
...
void plyr_play(char* file) {
    ...
    ESP_LOGD(TAG, "Play uri: %s", file);
    audio_element_set_uri(_fatfs_stream_reader, file);
...
void plyr_set_volume(int volume) {
    ...
    audio_hal_set_volume(_board->audio_hal, volume);
